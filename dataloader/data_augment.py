...
def _brightness(image, min=0.5, max=2.0):
    ...
    random_br = float(torch.Tensor(1).uniform_(min, max).numpy()[0])
...
def resize_image(image, gts, igs, scale=(0.4, 1.5)):
    ...
    ratio = float(torch.Tensor(1).uniform_(scale[0], scale[1]).numpy()[0])
...
def random_crop(image, gts, igs, crop_size, limit=8):
    ...
    if len(gts) > 0:
        sel_id = int(torch.randint(0, len(gts), (1,)).numpy()[0])
        sel_center_x = int((gts[sel_id, 0] + gts[sel_id, 2]) / 2.0)
        sel_center_y = int((gts[sel_id, 1] + gts[sel_id, 3]) / 2.0)
    else:
        sel_center_x = int(torch.randint(0, img_width - crop_w + 1, (1,)).numpy()[0] + crop_w * 0.5)
        sel_center_y = int(torch.randint(0, img_height - crop_h + 1, (1,)).numpy()[0] + crop_h * 0.5)
...
def random_pave(image, gts, igs, pave_size, limit=8):
    ...
    pave_x = int(torch.randint(0, pave_w - img_width + 1, (1,)).numpy()[0])
    pave_y = int(torch.randint(0, pave_h - img_height + 1, (1,)).numpy()[0])
...
def augment(img_data, c, img):
    ...
    # random brightness
    if c.brightness and int(torch.randint(0, 2, (1,)).numpy()[0]) == 0:
        img = _brightness(img, min=c.brightness[0], max=c.brightness[1])
    # random horizontal flip
    if c.use_horizontal_flips and int(torch.randint(0, 2, (1,)).numpy()[0]) == 0:
        img = cv2.flip(img, 1)
